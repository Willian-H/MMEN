...
class V4():
    ...
    def do_train(self, model, dataloader):
        ...
        # 记录每个epoch的各模态任务损失
        losses = {'M': [], 'T': [], 'A': [], 'V': []}
        lambda_weight = {'M': [], 'T': [], 'A': [], 'V': []} 
        T = 1.0 # temperature
        K = len(self.args.tasks)
        ...
        while True: 
            # apply Dynamic Weight Average
            weight = {'M': 0.0, 'T': 0.0, 'A': 0.0, 'V': 0.0}
            if epochs == 0 or epochs == 1:
                for m in self.args.tasks:
                    lambda_weight[m].append(1.0)
            else:
                # weight 衰减系数 前二个epoch的loss与前一个epoch的loss
                for m in self.args.tasks:
                    weight[m] = losses[m][epochs - 1] / losses[m][epochs - 2]
                total = 0.0
                for m in self.args.tasks:
                    total += np.exp(weight[m] / T)
                for m in self.args.tasks:
                    lambda_weight[m].append(K * np.exp(weight[m] / T) / total)
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    with autocast():
                        ...
                        loss = 0.0
                        for m in self.args.tasks:
                            loss_ori = self.criterion(outputs[m], labels[m])
                            losses[m].append(loss_ori.item())
                            #  加权
                            loss += lambda_weight[m][epochs] * loss_ori
                        ...
                        for m in self.args.tasks:
                            loss_mix = self.criterion(outputs_mix[m], labels[m])
                            losses[m][epochs] += loss_mix.item()
                            #  加权
                            loss += lambda_weight[m][epochs] * loss_mix
