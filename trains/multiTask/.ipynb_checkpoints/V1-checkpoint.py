            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    for m in self.args.tasks:
                        loss_mix += eval('self.args.'+m) * self.criterion(outputs_mix[m], mix_labels[m])
                    # weighted sum loss  
                    # for m in self.args.tasks:
                    #     loss_mix += model.Model.weights[m] * self.criterion(outputs_mix[m], mix_labels[m])                   
                    # compute loss
                    loss = 0.0
                    loss += loss_mix
...
#                         loss_V_mix += eval('self.args.'+m) * self.criterion(outputs_V_mix[m], labels[m])
                    ...
                    # for m in ['A','T','V']:
                    #     loss += 0.1 * eval('self.args.'+m) * self.criterion(outputs['M'], labels[m])
