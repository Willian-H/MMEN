...
class V6(nn.Module):
    def __init__(self, args):
        ...
        self.post_text_layer_1 = nn.Linear(self.text_hidden, 1)

        # define the classify layer for audio
        self.post_audio_dropout = nn.Dropout(p=self.post_audio_prob)
        self.post_audio_layer_1 = nn.Linear(self.audio_hidden, 1)

        # define the classify layer for video
        self.post_video_dropout = nn.Dropout(p=self.post_video_prob)
        self.post_video_layer_1 = nn.Linear(self.video_hidden, 1)
        ...
        self.fusion_network = TransFN(embed_dim=768, n_head=4, n_layers=3, mlp_ratio=2, drop_prob=0.1)
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        Args:
            ...
            text_x: tensor of shape (batch_size, sequence_len, text_in)
        '''
        text_x, _text = self.text_model(text_x)
        ...
        # text
        x_t1 = self.post_text_dropout(text_h)
        output_text = self.post_text_layer_1(x_t1)
        
        # audio
        x_a1 = self.post_audio_dropout(audio_h)
        output_audio = self.post_audio_layer_1(x_a1)
        
        # video
        x_v1 = self.post_video_dropout(video_h)
        output_video = self.post_video_layer_1(x_v1)
