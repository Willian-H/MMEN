...
@torch.no_grad()
def arbitrary_segment_mask(start_index: torch.Tensor, end_index: torch.Tensor, len_out: int, reverse: bool = False,
...
class Deformable_Attention(nn.Module):
    ...
    def _reset_specially(self):
        ...
        duration_init = torch.arange(self.init_duration_min, self.init_duration_max + 1, \
                                     step=(self.init_duration_max - self.init_duration_min) / (
                                                 self.num_heads - 1)) / self.length
    ...
    def forward(self, x: torch.Tensor):
        ...
        # (b nh) t t
        neg_bound_weight = neg_bound_l_dist_l[:, :, None] * bound_l_mask_l + neg_bound_r_dist_r[:, :,
                                                                             None] * bound_r_mask_r
...
class DST_Encoder(nn.Module):
    def __init__(self, embed_dim, ffn_embed_dim, num_heads, length, duration_init_min, duration_init_max, dropout=0.,
                 bias=True, activation='relu'):
        super().__init__()
        self.dropout = dropout

        self.attention = Deformable_Attention(embed_dim, num_heads, length, duration_init_min, duration_init_max,
                                              dropout, bias)
...
class DST(nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList(
            [DST_Encoder(input_dim, ffn_embed_dim, num_heads, length, duration_init_min, duration_init_max, dropout,
                         bias, activation)
...
class DST_Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        feature_maps = [x]  # Store feature maps from each layer

        for layer in self.layers:
            x = layer(x)
            feature_maps.append(x)  # Store the feature maps

        return x, feature_maps


# # Example of how to use the modified backbone:
# # Instantiate the backbone
# backbone = DST_Backbone(num_layers=3)  # Change the number of layers as needed
#
# # Pass your input data through the backbone
# input_data = torch.randn(1, 1024, 326)  # Replace with your input shape
