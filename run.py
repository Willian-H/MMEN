...
def run(args):
    ...
    # 当使用GPU1训练时，torch.cuda.empty_cache()默认是给GPU0释放显存，而此时GPU0接近跑满，且无显存可释放，直接使用这句话就会导致显存溢出
...
#                 min_mem_used = mem_used
#                 dst_gpu_id = g_id
#         print(f'找到 GPU: {dst_gpu_id}, 使用内存: {min_mem_used}!')
#         logger.info(f'找到 GPU: {dst_gpu_id}, 剩余内存: {min_mem_used}!')
#         args.gpu_ids.append(dst_gpu_id)
#
#     using_cuda = len(args.gpu_ids) > 0 and torch.cuda.is_available()
#     logger.info("使用 %d 个 GPU!" % len(args.gpu_ids))
#
#     if using_cuda:
...
#         return answer
#
#     logger.info(f'模型具有 {count_parameters(model)} 个可训练参数')
#     atio = ATIO().getTrain(args)
#
#     dataloader = MMDataLoader(args)
#
...
def parse_args():
    ...
    parser.add_argument('--modelName', type=str, default='v8',
